...
\begin{document}
	...
	\end{exmp}
	\begin{exmp}
		Given a directed graph $D=(V,E,E \xrightrightarrows[t]{s} V)$, we can create its free category $FD$, where the objects are the vertices and $FD(v,w)=\{\text{finite paths } v\rightarrow\ldots\rightarrow w\}$. We set $\id_v$ to be the path of length 0, while composition is just the concatenation of paths.
	\end{exmp}
	\section{Monads}
	...
	\begin{minipage}{0.3\linewidth}
		...
		&\mathsf{C}\ar[d, Rightarrow, shorten <= 1em, shorten >= 1em, "\mu"]\ar[r, "T"]\ar[drr, bend right=26, "T"description]
		&\mathsf{C}\ar[dr, "T"]\ar[d, Rightarrow, shorten <= 1em, shorten >= 1em, "\mu"]\\
		\mathsf{C}
		\ar[rrr, "T"'] 
		\ar[ur, , "T"]
		&\phantom{.} &\phantom{.}&\mathsf{C}
			\end{tikzcd}
	\end{minipage}
\hspace{1.5cm}
=
	\begin{minipage}{0.3\linewidth}
			\begin{tikzcd}[row sep=1cm, column sep=1cm]
		&\mathsf{C}\ar[d, Rightarrow, shorten <= 1em, shorten >= 1em, "\mu"]\ar[r, "T"]
		&\mathsf{C}\ar[d, Rightarrow, shorten <= 1em, shorten >= 1em, "\mu"]\ar[dr, "T"]\\
		\mathsf{C}\ar[urr, bend right=26, "T"'description]
		\ar[rrr, "T"'] 
		\ar[ur, , "T"]
		&\phantom{.} &\phantom{.}&\mathsf{C}
		%
		% drawing 2-cells  
		%\arrow[Rightarrow, "\mu", from=A, to=K]
		%\arrow[Rightarrow, "\mu", from=F, to=G]
		\end{tikzcd}
		\end{minipage}
