...
\begin{document}
	...
	Notice that we always have a category $\C$ and some functor $T\colon\C\rightarrow\C$ with an action map $T\C\rightarrow\C$.
...
\end{center}
	...
	\begin{exmp}
		COnsider $W\colon\Set\rightarrow\Set$ given by $WX=\amalg_{n\in\N}\Pi_{k=1}^n X$. Multiplication $\mu_X\colon WWX\rightarrow WX$ is given by concatenation of words, while the unit $\eta_X\colon X\rightarrow WX$ is just $x\mapsto (x)$.
	...
	Now that we have introduced these structures, our aim is to characterize monadic functors, that is functors $U\colon\A\rightarrow\C$ which are equivalent to $U^T\colon T-Alg\rightarrow\C$ for some monad $(T,\mu,\eta)$ on $\C$.
...
\flushleft
	\begin{tikzcd}[row sep=.55cm, column sep=.55cm]
		&&\C\arrow[rd, "F"]\ar[d, Rightarrow, shorten <= .5em, shorten >= .01em, "\epsilon"']&&&&&&\C\arrow[rd, "F"]\ar[d, Rightarrow, shorten <= .5em, shorten >= .01em, "\epsilon"'] &&\\
		\C\arrow[r, "F"]&\D\arrow[ru, "U"] \arrow[rr, bend right, equal] &\phantom{.}& \D \arrow[r, "U"] & \C \arrow[r, "F"] & \D \arrow[r, "U"] & \C  \arrow[r, "F"] &\D\arrow[ru, "U"] \arrow[rr, bend right, equal] &\phantom{.}& \D \arrow[r, "U"] & \C
		\end{tikzcd}
			\begin{center}
				=				
			\end{center}
			...
			\begin{tikzcd}
		&&\C\arrow[rd, "F"]\ar[d, Rightarrow, shorten <= .5em, shorten >= .01em, "\epsilon"']&&\C\arrow[rd, "F"]\ar[d, Rightarrow, shorten <= .5em, shorten >= .01em, "\epsilon"']\\
		\C\arrow[r, "F"]&\D\arrow[ru, "U"] \arrow[rr, bend right, equal] &\phantom{.}& \D\arrow[ru, "U"] \arrow[rr, bend right, equal] &\phantom{.}& \D\arrow[r, "U"]&\C
		\end{tikzcd}	
	\end{center}
\begin{center}
	=
		\begin{tikzcd}[row sep=.55cm, column sep=.55cm]
		&            &            &                                    &\C  \arrow[rd, "F"]\ar[d, Rightarrow, shorten <= .5em, shorten >= .01em, "\epsilon"'] &            &            &                                    &  \C\arrow[rd, "F"]\ar[d, Rightarrow, shorten <= .5em, shorten >= .01em, "\epsilon"'] &            &  \\
		\C\arrow[r, "F"] & \D \arrow[r, "U"] & \C  \arrow[r, "F"] &  \D\arrow[ru, "U"] \arrow[rr, bend right, equal] &  \phantom{.}           &\D  \arrow[r, "U"] &\C  \arrow[r, "F"] & \D \arrow[ru, "U"] \arrow[rr, bend right, equal] &\phantom{.}             &\D  \arrow[r, "U"] & \C
		\end{tikzcd}
\end{center}
The unit axiom is witnessed by [\dots]
\end{proof}
...
\end{exmp}
There is always a comparison morphism $\D\xrightarrow{\overline{U}}UF-Alg$	s.t. 
\[
\begin{tikzcd}
...
\end{tikzcd}
\]
commutes. We set $\overline{U}f=(Ud,UFUd\xrightarrow{U\epsilon_d}Ud)=(Ud, U\epsilon_d)$. More generally, for a given functor $\D\xrightarrow{G}\C$ we can ask what do we need to get a lift $\overline{G}\colon\D\to T-Alg$.
\begin{defn}
...
commutes.
\end{defn}
Up to size, we get a category $T$-$\mathsf{act}(\D)$
