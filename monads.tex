...
that, we need to monad in $2$-categories. We have shown that $2\text{-}\Cat$ is a locally finitely presentable cosmoi. This means in particular
...
$W \iso Y \copw_{\A} W$. For this we need to compute the Klinski object of the monad in $[\Delta_{+}, \Cat]$. If it really is a weighted colimit,
this would be computed pointwise in $\Cat$. So we need to compute the Klinski object of $\_+[0] \from \Delta_{+} \to \Delta_{+}$ in $\Cat$.
...
mapped to adjoints nuder $\tau_*$ or something like that (wtf does this mean?).
...
\begin{itemize}
	...
	\item $\cV=\sSet_{\text{Segal}}$, Segal categories, Resz dg, cosimplicial
