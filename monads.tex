...
\begin{thm}[Realisation-Nerve Adjunction]
...
\begin{proof}
    ...
    commutes and $l_{\mu+1}\cdot\sigma_{S^\mu c}=l_\mu$.
	\[
		\begin{tikzcd}
    ...
    hence we only need $l_0^*\colon\B(S^\kappa c,b)\rightarrow\C(c,b)$ to be a
    ...
    $\C(c,b)\rightarrow\C(c,b)$, $f\mapsto\sigma^{-1}_b\cdot Sf$.
\end{proof}
...
\begin{proof}
    ...
    top map $\colim_{\I}F(a_{i})\to F(\colim_{\I}a_i)$ is an isomorphism, in
...
\begin{proof}
    ...
    the canonical cocone $(\A\downarrow (c,d,\gamma))$ exhibits $(c,d,\gamma)$
    as a colimit. In the arrow category $\cC^{[1]}$, $Fc\rightarrow d$ is a
    colimit of all $\kappa$-presentable pairs $c_0,c_1\in\C_\lambda$.
	\[
		\begin{tikzcd}
			c_0\ar[d]\ar[r]
			& Fc\ar[d] \\
