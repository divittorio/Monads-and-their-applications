...
\end{defn}
\begin{defn}
	Given $c\in\C$, we write $\A\slash c$ for the category with objects $\varphi\colon Ka\to c$ and morphisms $(a,\varphi)\to(a',\varphi')$ maps $f\colon a\to a'$ s.t.\ $\varphi'Kf=\varphi$.
\end{defn}
...
\begin{thm}\label{long thm}
	Let $\C$ be a cocomplete category, $\A$ a small category and $K\colon\A\to\C$ a fully faithful functor. Then $\widetilde{K}$ has a left adjoint given by $\Lan_yK\colon[\A^{\op},\Set]\to\C$. 
		\[
	\begin{tikzcd}[column sep=tiny]
	\A\ar[rr, "y"]\ar[dr, swap, "K"]
	&& \left[\A^{\op},\Set\right]\ar[dl, "\Lan_yK"] \\
	& \C\end{tikzcd}
	\]
	Moreover, the following conditions are equivalent:
	\begin{enumerate}
		\item $\widetilde{K}$ is fully faithful;
		\item $\forall c\in\C$, the canonical cocone on $\dom\colon \A\slash c\to\C$ exibits it as colimit of the diagram $\dom\colon(K\downarrow c)\to\C$;
	...
	\item the counit of $\Lan_yK\dashv\widetilde{K}$ is an isomorphism.
	\end{enumerate}
	\end{thm}
	\begin{defn}
		A full subcategory/fully faithful functor $K\colon\A\to\C$ is called \emph{dense} if $\widetilde{K}$ is fully faithful.
	\end{defn}
\begin{proof}[Proof of Theorem \ref{long thm}]
	We have $\displaystyle \Lan_yK(F)=\colim_{\A(-,a)\Rightarrow F}Ka$, hence
	\begin{align*}
	\C(\Lan_yK(F),c)&\cong\lim_{\A(-,a)\Rightarrow F}\C(Ka,c)\\
					&\Yoneda \lim_{\A(-,a)\Rightarrow F}\left[\A^{\op},\Set\right]\left(\A(-,a),\C(K-,c)\right)\\
					&\cong\left[\A^{\op},\Set\right]\left(\colim_{\A(-,a)\Rightarrow F}\A(-,a),\C(K-,c)\right)\\
					&\cong\left[\A^{\op},\Set\right](F,\C(K-,c))
	\end{align*}
	hence $\Lan_yK\dashv\widetilde{K}$. It remains to check the implications.
	\begin{itemize}[itemindent=36pt]
	\item[1.$\implies$2.] If $\widetilde{K}$ is fully faithful, it suffices to check that the image of the canonical cocone under $\widetilde{K}$ is a colimit. Since $K$ is fully faithful, this image is precisely the diagram $\A(-,a)\cong\C(K-,Ka)\Rightarrow\C(K-,c)$. Then $$\colim\widetilde{K}\circ\dom=\displaystyle\colim_{y\downarrow\C(K-,c)}\A(-,a)\cong\C(K-,c).$$This proves $2.$ and the fact that the colimit of $(K\downarrow c)\to\C$ is preserved by $\widetilde{K}\colon\C\to\left[\A^{\op},\Set\right]$.
	...
	\item[4.$\implies$5.] Let $\mathcal{B}$ be the class of objects $b$ in $\C$ s.t.\ $\epsilon_b\colon\Lan_yK\widetilde{K}(b)\to b$ is an isomorphism. The subcategory $\B$ spanned by $\mathcal{B}$ is closed under $K$-absolute colimits since they are preserved by $\widetilde{K}$ (by definition), by the left adjoint $\Lan_yK$ and by $\id_{\C}$. It remains to check that the counit at $a\in\A$ is an isomorphism. But $\widetilde{K}a=\C(K-,Ka)\cong\A(-,a)$, so $\Lan_yK(\A(-,a))\cong a$, where the last isomorphism is a consequence of the fact that $(y\downarrow\A(-,a))$ has a terminal object $\id\colon\A(-,a)\Rightarrow\A(-,a)$.
